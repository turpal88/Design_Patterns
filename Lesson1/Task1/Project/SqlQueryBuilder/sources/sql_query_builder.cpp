...
SqlSelectQueryBuilder& SqlSelectQueryBuilder::AddWhere(std::string key, std::string value){
	
	
	std::future<SqlSelectQueryBuilder&> ft = std::async(std::launch::deferred, [&key, &value, this]() -> SqlSelectQueryBuilder& {
		std::unique_lock<std::mutex> lk(mtx);
		cv.wait(lk, [this]() {return this->is_from_entered; });
		size_t it = this->q.final_query.size() - 1;
		
		std::string where_string = (std::regex_match(value, this->q.txt_regex)) ? " "  + key + "='" + value + "'" : " " + key + "=" + value;
		if (count_of_add_where_calls > 0) where_string = " AND" + where_string;
